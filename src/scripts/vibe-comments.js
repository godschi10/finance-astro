 ...
 *
 * PORT OF THE WORDPRESS PLUGIN'S OWN CLIENT SCRIPT.
 * Source: vibe-comments/public/js/vibe-comments.js (WP plugin, v3.20.x).
 * Everything below is byte-verbatim to that original EXCEPT the transport
 * seam, which now talks to the Cloudflare Worker API instead of WordPress:
 *   - request URLs use config.apiBase (the Worker origin) + REST paths,
 *     in place of the WordPress AJAX URL and its action parameter;
 *   - the old WP action / security-token fields are removed from every request;
 *   - the numeric WP post id is replaced by the post SLUG STRING
 *     (config.postSlug, written into the hidden `comment_post_ID` input);
 *   - the DOMContentLoaded bootstrap calls only the v1 init list.
 * Non-v1 sections (mentions, reply push, edit window, sort, search, pin,
 * notify, Q&A, Google auth, security-token/session refresh, live polling, bulk
 * reaction sync) are retained and defined but unreachable - their markup
 * never renders and they are never invoked.
 *
...
(function() {
    ...
    // ── PORT SEAM: Worker API base ─────────────────────────────────────
    // The WP AJAX URL no longer exists; config.apiBase is the
    // Cloudflare Worker origin, e.g. 'https://comments-api.gwill.workers.dev'.
    // Any trailing slash is trimmed so every path join is a single slash.
    ...
    function bellIcon(label) {
        // v3.20.28 (King LAW: "vibe comments design should not stick to one
        // brand - the plugin is NEUTRAL, each theme overrides"): the bell's
        // state ink must come from the theme's --vibe-primary token (fallback
        // neutral blue), NOT the tech theme's hardcoded green. Same glyph
        // logic: On = filled + ringing arcs + 'On'; Off = outline + strike +
        // 'Off'.
        var on = (label === str('bellOn', 'On'));
        var ink = 'var(--vibe-primary, #3b82f6)';
        var dim = 'var(--vibe-text-muted, #8b96a5)';
        var use = on ? ink : dim;
        var glyph = on
        ...
        return '<svg aria-hidden="true" focusable="false" viewBox="0 0 20 20" fill="none" stroke-width="1.8" stroke-linecap="round" width="14" height="14" style="color:' + use + ';vertical-align:-2px;margin-right:5px">' + glyph + '</svg>'
             + '<span style="color:' + use + ';font-weight:600;font-size:12px">' + label + '</span>';
    }
...
// ── v3.20.11 (King): SVG icon set - emoji glyphs ignore CSS color and
    ...
    const REACTION_SVG = {
            ...
            // v3.20.27 (King: "center the SVG inside the circle"): TRUE path
            // bbox walked from the served 'd' (x7.10..25.00 cx16.05 / y3.20..25.50
            // cy14.35) -> exact centering transform translate(9.57 10.59). Glyph
            // mass now dead-on (12,12) with ~2.4 units clearance all round.
    ...
    function buildSummaryInner(sorted, userReaction) {
        ...
        // v3.20.20 (King: FB Lite stacked reactions): each bubble carries a
        // .vibe-rx-t-<type> class so the theme skin paints Facebook's disc
        // colors (blue/red/gold/yellow). The glyph is ALWAYS the SVG set -
        // device emoji are intrinsically colored and CSS cannot make them
        // white-on-disc; the picker keeps emoji-first via rxIcon().
        var bubbles = sorted.slice(0, 3).map(function(r) {
            return '<span class="vibe-rx-bubble vibe-rx-t-' + r.def.type + (userReaction === r.def.type ? ' vibe-rx-mine' : '') + '">' + rxSvg(r.def.type) + '</span>';
        }).join('');
    ...
    function boot() {
        ...
        fetchCommentCount();
        initCommentsTrigger();
        // PORT SEAM (v1 bootstrap): only the v1 inits run. The held features
        // (mentions, reply push, edit window, sort, search, pin, notify, Q&A,
        // Google auth, security-token/session refresh, live polling, reaction sync)
        // stay defined below but are never invoked.
        initReactions();
        initReplies();
        initViewReplies();
        initRelativeTime();
    ...
    function fetchCommentCount() {
        ...
        if (!heading || !config.postSlug) return;

        var url = apiUrl('/count?post=' + encodeURIComponent(config.postSlug));
    ...
    function syncReactions() {
        ...
        // PORT SEAM: the WP bulk reaction-sync action has no v1 Worker
        // route. Guests get user_reaction embedded in the load response, and
        // config.isLoggedIn is always false on the static site, so this path
        // is retained but never reached (syncReactions() is not in the v1
        // bootstrap either).
        ...
        fetchWithTimeout(apiUrl('/comments/reactions'), {
            method:  'POST',
    ...
    function createCommentElement(comment) {
        ...
        const viewRepliesHtml = (hasReplyCount && !alreadyExpanded)
            ? '<button type="button" class="vibe-view-replies-btn" data-comment-id="' + cid + '" data-post-id="' + (config.postSlug || '') + '" data-state="collapsed">'
    ...
    function initReactions() {
        document.addEventListener('click', function(e) {
            ...
            if (option) {
                ...
                fetchWithTimeout(apiUrl('/comments/' + parseInt(commentId, 10) + '/reactions'), {
                    method:  'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
     ...
     * content div; Save posts the edit payload (vibe_guest_id for guests,
     * mirroring submit); the server alone decides authorization
    ...
    function initEditWindow() {
        document.addEventListener('click', function(e) {
            ...
            function saveEdit() {
                ...
                // PORT SEAM: no v1 Worker route for the edit window (P2-held).
                fetchWithTimeout(apiUrl('/comments/' + parseInt(btn.dataset.commentId, 10)), {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: params
                }, 15000)
    ...
    function initDraftSave() {
        ...
        if (!form || !textarea || !config.postSlug) return null;

        const DRAFT_KEY = 'vibe_draft_' + config.postSlug;
    ...
    function initForm() {
        ...
        form.addEventListener('submit', function(e) {
            ...
            const data = {
                // PORT SEAM: post_id is the SLUG STRING now - no parseInt().
                post_id: formData.get('comment_post_ID') || '',
                content: (formData.get('comment') || '').trim(),
                parent: parseInt(formData.get('comment_parent') || 0, 10),
                author: (formData.get('author') || '').trim(),
                email: (formData.get('email') || '').trim(),
                // PORT SEAM: the honeypot must ride the Worker submit body -
                // there is no PHP $_POST side-channel reading it off the form.
                vibe_hp: formData.get('vibe_hp') || '',
            };
            ...
            fetchWithTimeout(apiUrl('/comments'), {
                method: 'POST',
                headers: {
    ...
    /**
     * Lazy load: fetch page 1 from the comments API and render into the empty list.
    ...
    function initComments(onComplete) {
        ...
        if (!list || !config.postSlug) return;
        ...
        .then(function(result) {
            ...
            if (comments.length === 0) {
                ...
                // PORT SEAM (v1 bootstrap law): the sort/search toolbar is held
                // for P2 — initSortToggle() and initSearch() are deliberately NOT
                // invoked, so revealing the toolbar here would put an inert sort
                // button on screen. WP ships this markup hidden (comments.php:
                // style="display:none") and reveals it only from the sort code,
                // which v1 does not run: the toolbar therefore STAYS hidden.
    ...
    function initViewReplies() {
        document.addEventListener('click', function(e) {
            ...
            var postId    = btn.dataset.postId || config.postSlug;
            btn.dataset.collapsedLabel = btn.textContent; // remember "View N replies" for later
            btn.disabled    = true;
            btn.textContent = str('loading', 'Loading\u2026');

            var url = apiUrl('/comments/' + parseInt(commentId, 10) + '/replies?post=' + encodeURIComponent(postId));
            if (!config.isLoggedIn) { url += '&vibe_guest_id=' + encodeURIComponent(getGuestId()); }
    ...
    /**
     * PORT SEAM (dropped feature): WP security tokens are meaningless off WordPress
     * and the Worker API has no CSRF-token rail (Turnstile rides the submit
     * call instead). The function is retained for parity, is not part of the
     * v1 bootstrap, and deliberately performs no request.
     */
    ...
    /**
     * PORT SEAM (dropped feature): this reconciliation existed because a WP
     * page cache could bake anonymous isLoggedIn/isAdmin/qa flags into the
     * HTML. The static site has no sessions - config is the truth - so the
     * Worker API has no session route. The function is retained for parity,
     * is not part of the v1 bootstrap, and performs no request.
     */
    ...
    function initGoogleAuth() {
        ...
        btn.addEventListener('click', function() {
            ...
            // PORT SEAM: no v1 Worker route for Google auth - retained,
            // unreachable (the button never renders; googleEnabled is false).
            fetchWithTimeout(apiUrl('/auth/google'), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
    ...
    function initSearch() {
        ...
        function runServerSearch(q) {
            const seq = ++searchSeq;
            // PORT SEAM: no v1 Worker search route - the client's localFilter()
            // fallback below is what runs. Retained, not in the v1 bootstrap.
            const url = apiUrl('/comments?post=' + encodeURIComponent(config.postSlug))
    ...
    function initPinComment() {
        ...
        document.addEventListener('click', function(e) {
            ...
            btn.disabled = true;
            // PORT SEAM: no v1 Worker route for pin/unpin - retained,
            // unreachable (config.isAdmin is false on the static site).
            fetchWithTimeout(apiUrl('/comments/' + parseInt(id, 10) + '/pin'), {
                method:  'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body:    new URLSearchParams({
                    comment_id: id,
     ...
     * Click flips reply-email consent for that thread server-side (the same
     * ownership rail the other held features use) and updates the pill.
     *
     * PORT SEAM: no v1 Worker route - retained, unreachable (its markup never
     * renders and it is not in the v1 bootstrap).
     */
    function initNotifyToggle() {
        document.addEventListener('click', function(e) {
            ...
            fetch(apiUrl('/comments/' + parseInt(btn.dataset.commentId, 10) + '/notify'), {
                method: 'POST',
     ...
     * posts the accept toggle; the client then reflects it instantly: badge
     * swap on the affected answer,
    ...
    function initQAAccept() {
        document.addEventListener('click', function(e) {
            ...
            // PORT SEAM: no v1 Worker route for Q&A accept - retained,
            // unreachable (config.qa is false on the static site).
            fetch(apiUrl('/comments/' + parseInt(commentId, 10) + '/accept'), {
                method: 'POST',
                credentials: 'same-origin',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: new URLSearchParams({
                    comment_id: commentId,
                    post_id:    config.postSlug || ''
                })
            })
            .then(function(r) { return r.json(); })
            .then(function(res) {
