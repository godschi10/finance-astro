...
(function () {
	...
	const L_I18N = (typeof window.GwillLightbox !== 'undefined' && window.GwillLightbox.i18n) || {};
	...
	// ── 11. Image lightbox (lightbox.js, whole file) ────────────────
	// Enqueued `is_singular()` only (enqueue.php:169-193). Delegated
	// listeners on `document`, lazy overlay DOM, body overflow locked while
	// open. Ported whole, gated on `.art-body` existing so it is inert on a
	// page with no article body.
	let lbOverlay, lbImg, lbCloseBtn, lbPrevBtn, lbNextBtn, lbCounterEl, lbCaptionEl;
	let lbImages, lbIndex;
	...
	const lbBuild = () => {
		...
		lbOverlay.className = 'gl-overlay';
		lbOverlay.setAttribute('role', 'dialog');
		lbOverlay.setAttribute('aria-modal', 'true');
		lbOverlay.setAttribute('aria-label', L_I18N.lightbox || 'Image lightbox');
		lbOverlay.innerHTML =
		...
		lbImg = lbOverlay.querySelector('.gl-image');
		lbCloseBtn = lbOverlay.querySelector('.gl-close');
		lbPrevBtn = lbOverlay.querySelector('.gl-prev');
		lbNextBtn = lbOverlay.querySelector('.gl-next');
		lbCounterEl = lbOverlay.querySelector('.gl-counter');
		lbCaptionEl = lbOverlay.querySelector('.gl-caption');

		lbCloseBtn.addEventListener('click', lbClose);
		lbOverlay.addEventListener('click', (e) => {
			if (e.target === lbOverlay) lbClose();
		});
		lbPrevBtn.addEventListener('click', (e) => { e.stopPropagation(); lbNavigate(-1); });
		lbNextBtn.addEventListener('click', (e) => { e.stopPropagation(); lbNavigate(1); });

		document.addEventListener('keydown', (e) => {
			if (!lbOverlay.classList.contains('gl-open')) return;
			if (e.key === 'Escape') lbClose();
			if (e.key === 'ArrowLeft') lbNavigate(-1);
			if (e.key === 'ArrowRight') lbNavigate(1);
			// Tab trap (WCAG 2.4.3): keep focus inside the dialog.
			if (e.key === 'Tab') {
				const focusables = Array.prototype.filter.call(lbOverlay.querySelectorAll('button'), (b) => {
				...
				if (e.shiftKey) {
					if (document.activeElement === first || !lbOverlay.contains(document.activeElement)) {
						e.preventDefault(); last.focus();
					}
				} else {
					if (document.activeElement === last || !lbOverlay.contains(document.activeElement)) {
		...
		// Touch / swipe: horizontal travel > vertical AND > 40px navigates.
		let touchStartX = 0;
		let touchStartY = 0;
	...
	const lbOpen = (imgEl, index) => {
		lbBuild();
		lbImages = lbGatherImages(imgEl);
		lbIndex = index !== undefined ? index : lbImages.indexOf(imgEl);
		if (lbIndex === -1) lbIndex = 0;
		lbLastTrigger = imgEl;
		lbShowImage();
		lbOverlay.classList.add('gl-open');
	...
	const lbClose = () => {
		lbOverlay.classList.remove('gl-open');
		document.body.style.overflow = '';
		// Return focus to the triggering image (WCAG 2.4.3).
		if (lbLastTrigger && lbLastTrigger.focus) { lbLastTrigger.focus(); lbLastTrigger = null; }
	};
	...
	const lbShowImage = () => {
		if (!lbImages || !lbImages[lbIndex]) return;
		const el = lbImages[lbIndex];
		const src = el.getAttribute('data-full') || el.src || el.getAttribute('data-src') || '';
		// Use the largest available src: `data-full-url` first, then the LAST
		// candidate of srcset (WP lists sizes ascending, largest last).
		let fullSrc = el.dataset.fullUrl || el.src;
		const srcset = el.srcset || '';
		if (srcset) {
			const sources = srcset.split(',');
			const last = sources[sources.length - 1].trim().split(' ')[0];
		...
		const cap = fig ? fig.querySelector('.wp-element-caption, figcaption') : null;
		lbCaptionEl.textContent = cap ? cap.textContent : '';

		// Counter only when there is more than one image.
		lbCounterEl.textContent = lbImages.length > 1 ? (lbIndex + 1) + ' / ' + lbImages.length : '';

		// Nav visibility.
		lbPrevBtn.style.display = lbImages.length > 1 ? '' : 'none';
		lbNextBtn.style.display = lbImages.length > 1 ? '' : 'none';
	};

	// Presentational-image guard: decorative images (cover backgrounds,
	// aria-hidden, role=presentation, empty alt with no title) stay OUT of the
	// interaction tree — marking them role=button made it ambiguous.
	const lbIsPresentational = (img) => {
	...
	// Inside a gallery block: that gallery's images. Otherwise: every image in
	// the enclosing .art-body, so prev/next walks the whole article.
	const lbGatherImages = (startEl) => {
		const gallery = startEl.closest('.wp-block-gallery');
		if (gallery) {
			return Array.from(gallery.querySelectorAll('img')).filter((i) => !lbIsPresentational(i));
		}
		const prose = startEl.closest('.art-body');
		if (prose) {
			return Array.from(prose.querySelectorAll('img')).filter((i) => !lbIsPresentational(i));
		}
		return [startEl];
	};

	const lbInit = () => {
		if (!document.querySelector('.art-body')) return; // no article body, nothing to do
		...
		// The alt stays in the accessible name (prefix + alt, never a bare
		// "Enlarge image" that would swallow it).
		const enlarge = L_I18N.enlarge || 'Enlarge image';
		Array.prototype.forEach.call(document.querySelectorAll('.art-body img'), (el) => {
			if (el.closest('a')) return; // already linked, skip
			if (lbIsPresentational(el)) return; // decorative, keep out of the a11y tree
		...
		document.addEventListener('keydown', (e) => {
			const t = e.target;
			if ((e.key === 'Enter' || e.key === ' ') && t && t.tagName === 'IMG' && !lbIsPresentational(t) && t.closest('.art-body') && !t.closest('a')) {
				e.preventDefault();
				const all = lbGatherImages(t);
				const idx = all.indexOf(t);
				lbOpen(t, idx !== -1 ? idx : 0);
			}
		});

		document.addEventListener('click', (e) => {
			// Only handle clicks on images inside .art-body.
			const target = e.target.closest('.art-body img');
			if (!target) return;
			if (lbIsPresentational(target)) return;
			if (target.closest('a')) return; // already linked
			e.preventDefault();
			const allImgs = lbGatherImages(target);
			const idx = allImgs.indexOf(target);
			lbOpen(target, idx !== -1 ? idx : 0);
		});
	};

	// Kick off after DOM (deferred script => readyState is already
	// 'interactive', so this runs immediately, exactly like the theme).
